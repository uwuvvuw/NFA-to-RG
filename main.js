...
function checkStrings(input, output){
    ...
    const checkStrings = (graph, sequence, start, goal, goal1, goal2, goal3, goal4) => {
        ...
        while (stack.length > 0) {
            ...
            if (current === (goal || goal1 || goal2 || goal3 || goal4 )) return 'ACCEPTED';
...
function checkString(input, output){
  ...
  let currState = nfa.initialState
  let tran = nfa.transitions
  let final = nfa.finalStates
  let flag = 0
  ...
  do{
    ...
    for(let k=0; k<final.length;k++){
      console.log("current "+currState + " final "+final[k])
      if(currState === final[k]){
        return true
      }
    }
    ...
    for(let j=0; j<tran.length;j++){
      ...
      if(currState == tran[j].state && temp[0] == tran[j].symbol){
        currState = tran[j].nextStates[0]
        temp.shift()
        
      }
      else if (currState == tran[j].state && tran[j].symbol == "\u03BB"){
        currState = tran[j].nextStates[0]
        ...
        // if(temp[0] == tran[j].symbol){
        //   currState = tran[j].nextStates[0]
        //   temp.shift()
        // }
        // else if(tran[j].symbol == "\u03BB"){
        //   currState = tran[j].nextStates[0]
        // }
        // else if(temp[0] != tran[j].symbol){
        //   continue
        // }
      

      for(let k=0; k<final.length;k++){
        console.log("current "+typeof(currState) + " final "+typeof(final[k]))
        if(currState === final[k]){
          console.log("????????????")
          return true
        }
